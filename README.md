...
최신 로컬 개발 ISO(v5, SHA-256 `50e3a385fda7347347c2d4cfb9632a62a2486f92a1e1e93ac21e0b9f2fea7630`)에서 **첫 번째 스테이지만** 순회했습니다. 미션 1의 제목과 설명 두 줄이 한국어로 표시됐고, 지도 라벨·모드 설명·안내 패널도 정상이었습니다. 다른 스테이지와 실기 PSP는 검수하지 않았습니다. 이 ISO는 저장소에 포함하지 않으며 배포용도 아닙니다. 이미지 글자는 후속 작업으로 미뤘습니다.
...
- ASCII: 1바이트 그대로 사용. 다만 반각 인쇄 가능 ASCII(0x21~0x7E)가 화면에 그려지지 않는 문제가 확인되어 전각 매핑 수정안을 검증 중입니다.
